...
export function FeatureBlocks() {
  return (
    <Section bg="linen" size="compact">
      ...
      <div className="mt-10 grid grid-cols-1 gap-x-8 gap-y-8 md:grid-cols-3">
        {homeFeatureBlocks.map((block) => (
          <div key={block.title} className="flex flex-col">
            ...
            <p className="mt-2 text-sm leading-relaxed text-iron-grey">{block.description}</p>
            <Link
              href={block.ctaHref}
              className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-pine-smoke underline underline-offset-4 transition-colors hover:text-moss-oak"
            >
              {block.ctaLabel}
